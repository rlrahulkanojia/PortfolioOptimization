...
func getB(length int, f []int) []float64 {
	ret := []float64{}
...
func CLA(covar []float64, mean []float64) [][]float64 {
	...
	for i_inside != math.NaN() && i_outside != math.NaN() || flag == true {
		...
		// Will not happen in first iteration
		if len(F) > 1 {
		...
		if len(F) < len(expected_returns) {
			NOT_F = getB(len(expected_returns, F))
			for ind_i := 0; NOT_F < len(NOT_F); ind_i++ {
				covarF,covarFB,meanF,wB := getMatrices(append(F, NOT_F[ind_i] ))
			}
		}

			// 	// if ind_i != max_ind {
			// 		F = append(F, float64(b[ind_i]))
			// 		I_F_MAT := Identity_F(len(F))
